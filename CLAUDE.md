...
> **Last reconciled with code:** 2026-09-22 (**5 Alive went from three decks to five, and from 751 categories to 1,673.** Easy 300 -> 499, Hard 300 -> 427, Social 151 -> 267, plus two new decks: **Desi** (240 — India: food, films, cricket, shaadi, festivals) and **Kids** (240 — family-safe). The Hard pool was also cleaned: 24 near-duplicates the eye misses at 300 entries ("Toolbox items" vs "Things in a tool box") and 17 prompts too vague to name five of under a 6-second clock. The decks are a flat `{ key: string[] }` map joined to `DIFFICULTY_TILES` in `FiveAliveGame.tsx` **only by matching string keys** — a tile with no pool does not crash, it deals five blank cards — so `tests/fiveAliveCategories.test.ts` pins that parity in CI along with the content rules (no near-duplicate inside a deck, easy/hard/spicy mutually exclusive, <=34 chars). `scripts/drive-five-alive-decks.mjs` plays a turn on each deck and checks every dealt category back against its pool.)
>
...
| **5 Alive** | `FIVE_ALIVE` | Name N in N seconds, beat the bell | None (offline) | 5 descending rounds — name 5/4/3/2/1, timed 6/5/4/3/2s (extra second to read the clue) — perfect-round bonus, judge tallies. **Five decks, 1,673 categories, in `src/data/five_alive.json`**: Easy (499, broad — everyone can play), Hard (427, letter/word constraints + specialist recall), **Desi** (240 — India: food, films, cricket, shaadi, festivals), **Kids** (240 — family-safe, nothing a child can't name) and Social (267, adult-gated behind PIN 0438). The decks are a flat `{ key: string[] }` map and the picker is `DIFFICULTY_TILES` in `FiveAliveGame.tsx`; **the two are joined only by matching string keys**, and a tile with no pool does NOT crash — `drawTurnCategories` falls back to `[]` and deals five blank cards, which is invisible to the build, the typecheck and a drive that only opens Easy. `tests/fiveAliveCategories.test.ts` pins that parity in both directions **in CI**, plus the content rules: no near-duplicate inside a deck (the normalizer catches "Toolbox items" against "Things in a tool box"); easy/hard/spicy mutually exclusive, because a player moves between them looking for a different game; desi/kids mutually exclusive but free to reuse a general category, since they are themed selections off the same everyday ground; ≤34 chars so the card doesn't truncate. **Adding a deck = one pool key + one tile with the same key, nothing else.** End-of-round bell + tick synthesized via Web Audio (no bundled assets); the landing screen uses the shared compact `TeamRosterRow` (collapsed gold prompt) for optional player names (persists across games via the shared session team store), difficulty picked after. Also has a "Just Play" no-scoring mode. |
...
tests/fiveAliveCategories.test.ts    # vitest: 5 Alive deck-tile/pool parity + per-deck category hygiene (run by CI)
...
scripts/drive-five-alive-decks.mjs   # DEV-only: a full turn on each of 5 Alive's five decks, every category checked back against its pool
scripts/_drive-kit.mjs               # Shared drive mechanics: launch, page+console/dialog wiring, PIN gate, NEW tab. Every drive imports it.
```
