...
| Would You Rather | `WOULD_YOU_RATHER` | Paired dilemmas | Local static data | **Live since 2026-09-23** (was Coming Soon). One general deck, 100 hand-written cards in `src/data/would_you_rather.json`, dealt 10 a round with session dedupe, then a round-end screen ("went with the crowd on N of 10" + Ten more). Tap a side → both splits show and one line says whether you're with the majority. **The splits are authored estimates, not recorded votes** — the card's footnote says exactly that; never relabel them "player votes". The old per-card "psychoanalysis" roast was removed at the founder's request. Decks are data (`name`/`tagline`/`color`/`adult` per category in the JSON) — add a second deck and the picker appears on its own; an `adult` deck gets the 0438 gate. Every card carries a `tag` (social/life/powers/money/truth/food/phone/people/weird) for a future split into decks. THE DILEMMA BAR (`tests/wouldYouRather.test.ts`): splits sum to 100 and sit within 20–80 (a 90/10 card is not a dilemma), no option text repeats, options ≤80 chars. The `wyr_batch` AI handler still exists but nothing in the game calls it. |
...
tests/wouldYouRather.test.ts         # vitest: THE DILEMMA BAR — WYR deck shape, 20–80 splits, no repeated options (run by CI)
...
scripts/drive-would-you-rather.mjs   # DEV-only: two rounds of Would You Rather checked card-by-card against the JSON
