...
> **Last reconciled with code:** 2026-09-05 (added two brain games as **top-level Home games**: `src/components/games/BallparkGame.tsx` + `src/data/ballpark.json` — "Ballpark", an estimation/calibration game and the app's second solo-capable game; and `src/components/games/EchoGame.tsx` + `src/data/echo.json` — "Echo", a memory-chain game with a sabotage layer. Also added `scripts/drive-ballpark.mjs` and `scripts/drive-echo.mjs`, `target` + `layers` keys to `getIcon`, both games to `scripts/drive-games.mjs`, and — fixing two documented-intent drifts found on the way — House Rules' stats id (`house_rules` → `HOUSE_RULES`) and its absence from Game Night's eligible list.) If you're reading this and something in the codebase doesn't match what's described here, **the code is the source of truth** — please update this file in the same PR that makes the change.
>
...
- **Browser regression drives (dev-only, not in CI):** `scripts/drive-games.mjs` (opens the 15 Play Now games headless — all 19 with `--tabs` — and fails on console errors) and `scripts/deep-drive.mjs` (countdown/expiry/score flows in the 6 timer games) and `scripts/drive-the-tell.mjs` (plays a full 12-round game of The Tell and asserts every outcome branch) and `scripts/drive-nerve.mjs` (plays a best-of-3 of Nerve and asserts the ladder-escalation invariant) and `scripts/drive-house-rules.mjs` (plays a 9-law session and checks the app's scoring against an independently-computed tally) and `scripts/drive-ballpark.mjs` (a 3-player and a solo game, every expected score recomputed from the JSON) and `scripts/drive-echo.mjs` (asserts the chain-growth invariant at every replay) against — the last two draw randomised content, so run them a few times — `npm run build && npx vite preview --port 4173`. See `notes/02-browser-regression-drive.md` for the gotchas. Run these after touching shared game code.
...
│   └── games/                       # One file per game (incl. JumbleGame = "Scramble", IntimateDiceGame, TheTellGame, NerveGame, HouseRulesGame, BallparkGame, EchoGame)
├── contexts/
