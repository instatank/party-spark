...
> **Last reconciled with code:** 2026-09-25 (**Rank Me ships — a new social reading game, first in the NEW tab.** A ranker privately drags five items into their true order; somebody else predicts it; the reveal joins the two orders with lines coloured exact / one off / miss. Three modes on one dataset: **Hot Seat** (3+, the room agrees one guess), **Know Me** (exactly 2, 6/8/10 cards, roles swap) and **Couples vs Couples** (pairs entered side by side, same card for every team each round, leaderboard after each round). 160 founder-authored cards in `src/data/rank_me.json`, generated by the committed `scripts/build-rank-me.py`. Scoring is ends-weighted (exact 2 / one off 1 / further 0, the ranker's #1 and #5 count double, max 14) and lives ONCE in `src/services/rankMeEngine.ts` with the dealer; `tests/rankMe.test.ts` pins the brief's reference table and checks all 120 permutations. `scripts/drive-rank-me.mjs` plays all three modes and re-scores every reveal independently.)
>
> **Earlier 2026-09-24** (**Would You Rather: HOT SEAT mode + 20 exciting Couples cards.** A Classic / 🔥 Hot Seat switch on the deck picker. Hot Seat needs 2+ names (the shared `TeamRosterRow`, so the roster carries across games): one player picks **in secret**, the room argues and taps its call, then the pick is revealed with the splits and a "room's call" marker; the seat rotates through the roster and carries across rounds; the round-end screen names the hardest to read ("fooled the room N of M"). Three taps a card — that is the mode; Classic's one-tap loop is untouched. **The secret tap must leave no trace** (no gold, no split, no verdict line — the verdict is only rendered once revealed); `scripts/drive-would-you-rather.mjs` asserts that on every card and checks the round-end tally against its own. Couples grew 55 → 75 with date-night adventure cards, taking its "exciting" count from 2 to 21.)
>
...
- **Games / NEW tabs:** the newest games (`NEW_GAME_IDS` in `App.tsx` — Rank Me, Would You Rather, The Line, Target, Shortlist, Echo, Ballpark, House Rules) are split out of the main list into their own **NEW** tab above the filter pills, so the front page stays a short list. The tab strip hides itself while a search is running, because **search deliberately spans both tabs** — a game the user typed the name of must never come back "no games match" because it sits on the other tab. Moving an id out of `NEW_GAME_IDS` returns that game to the main list, nothing else to change. This is separate from the `SHOW_TABS` Play Now / Coming Soon bar above. **Dev drives must click the NEW tab** (`button[aria-label="New games"]`) before those eight game cards exist in the DOM — `drive-games.mjs` and the per-game drives already do.
...
| **Rank Me** | `RANK_ME` | Rank five things; someone predicts your order | None (offline) | A social reading game, not trivia: the only right answer is the ranker's own. **Three modes, one dataset, one code path** — they differ only in who ranks and who reads (`buildTurns` in `RankMeGame.tsx`): **Hot Seat** (3+ names; the seat rotates every card and carries across rounds; the ROOM reads and scores; a round is two laps up to 5 players, one lap from 6), **Know Me** (exactly 2; 6/8/10 cards, default 8; roles swap every card; the end screen says who knows whom better, splits the score by deck, and shows the best read and biggest miss with both orders), **Couples vs Couples** (4, 6, 8… names, partners entered side by side and paired in order; every team plays the SAME card each round; roles alternate inside each team; leaderboard after every round; default 6 rounds, 4/6/8). Teams are `string[][]` so bigger teams are a change to `pairUp` alone. Flow per turn: hand-off → private ranking → hand-off → prediction → reveal; the hand-off screens show no items, which is the privacy. **Scoring (`rankMeEngine.ts`, identical in every mode):** exact spot 2, one spot off 1, further 0; the items the ranker put at #1 or #5 count double; max 14, shown as points and %; tiers Mind reader 14 / Close 10–12 / Getting there 7–9 / Guesswork 0–6 (13 is unreachable). **Points always go to the reader, never to the ranker for being hard to read** — don't add a "fooled them" reward; it would pay people to rank dishonestly. **Dealing:** players multi-select decks; After Dark is `spicy` and needs the 0438 unlock (the tile shows a lock and asks for the PIN; `cardPool` also refuses spicy cards without the unlock, so a stale selection can't leak one); **Keep it sweet** drops `ex: true` cards; no repeats in a session; a device-level `rank_me_seen` list in localStorage makes unseen cards preferred across sessions and is quietly reset per pool when it runs dry; the same `theme` is never dealt twice in a row when another is available. **Every ranking screen shuffles its own starting order**, and the reader never starts on the ranker's answer (`startingOrder`), or a no-touch lock-in would score 14. Reordering: drag (pointer events, `touch-action: none`, one code path for mouse/touch/pen), tap two to swap, or arrow keys. Data: `src/data/rank_me.json` — 160 cards (Real Life 80, What If 40, After Dark 40) authored by the founder and **generated by `scripts/build-rank-me.py`**, which also validates it; edit the script and re-run it, never the JSON by hand. The data's `theme` is fine-grained only in Real Life (food/life/peeves/downtime); What If and After Dark carry almost one theme each, so the no-repeat-theme rule has nothing to separate there. `desi` is in the data but not yet used. Reuses `TeamRosterRow`, `PinGateModal`, `statsStore` (plays; wins for the Know Me / Couples winner), `firstPlay`, `audio` / `haptics`. Not in Game Night (it would need a `reportResult` shape for three modes). |
...
│   ├── rankMeEngine.ts              # Rank Me: ends-weighted scoring, deck pool, dealer (no repeats / prefer unseen / vary theme), device seen-store
...
tests/rankMe.test.ts                 # vitest: Rank Me's scoring reference table + all 120 permutations, deck shape, gating / sweet / no-repeat / unseen / theme dealing (run by CI)
...
scripts/drive-games.mjs              # DEV-only headless-browser drive: opens the 20 Play Now games (8 via the NEW tab; 23 with --tabs), fails on console errors
...
scripts/drive-rank-me.mjs            # DEV-only: all three Rank Me modes end to end, every score re-derived independently
scripts/build-rank-me.py             # Generates + validates src/data/rank_me.json (the founder's authoring script)
