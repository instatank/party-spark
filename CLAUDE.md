...
| **Jumble** | `JUMBLE` | Find as many words as possible from 7 scrambled letters before the timer | None (offline) | PartySpark's first true **solo** game (also Pass-and-Play). **No authored content + no dictionary shipped** — a dev script (`scripts/build-jumble-sets.mjs`) runs the ENABLE word list (172k inflected words, cached gitignored under `scripts/.cache/`) once and bakes 150 easy + 150 hard 7-letter sets, each with its FULL answer key, into `src/data/jumble_sets.json` (~170KB, lazy-loaded via dynamic import so it's code-split out of the initial bundle). At play time validation = O(1) answer-key lookup + a local formability check; zero API, fully offline. Engine in `src/services/jumbleEngine.ts`. Easy = any word; Hard = every word must use the amber **center** tile. Length-weighted scoring (3/4/5/6/7 = 1/2/4/6/10; 7-letter = pangram + celebration). User-set timer (30/60/90/120/custom 15–300s, persisted to localStorage). **Solo**: beat-your-best (localStorage per difficulty), end screen shows found + high-value missed words + pangram. **Pass and Play** (2–8): same letters + timer for all, pass-to-next gate, **unique-word scoring** (words found by 2+ players cancel) + leaderboard. To refresh/resize the set pack, re-run the build script (needs the dictionary; fetch it to `scripts/.cache/enable1.txt` if missing). |
