...
> **Last reconciled with code:** 2026-09-24 (**Would You Rather: HOT SEAT mode + 20 exciting Couples cards.** A Classic / 🔥 Hot Seat switch on the deck picker. Hot Seat needs 2+ names (the shared `TeamRosterRow`, so the roster carries across games): one player picks **in secret**, the room argues and taps its call, then the pick is revealed with the splits and a "room's call" marker; the seat rotates through the roster and carries across rounds; the round-end screen names the hardest to read ("fooled the room N of M"). Three taps a card — that is the mode; Classic's one-tap loop is untouched. **The secret tap must leave no trace** (no gold, no split, no verdict line — the verdict is only rendered once revealed); `scripts/drive-would-you-rather.mjs` asserts that on every card and checks the round-end tally against its own. Couples grew 55 → 75 with date-night adventure cards, taking its "exciting" count from 2 to 21.)
>
> **Earlier 2026-09-24** (**Would You Rather: THE FUN BAR.** The founder found the expanded deck too plain in places and set a bar: every card must be edgy, funny, exciting or revealing. An audit failed 84 of 255 cards on all four (66 in Friends & Family, mostly logistics dilemmas like laundry-vs-dishes; 14 Couples; 4 Spicy); all 84 were rewritten IN PLACE so their ids — the session-dedupe keys — survive. Every card now carries `hooks` naming the bar(s) it clears, and `tests/wouldYouRather.test.ts` fails a card with none. The label is an authoring judgement, not a measurement; the test only stops a card shipping with no claim at all.)
>
> **Earlier 2026-09-24** (**Would You Rather split into three room-shaped decks** — Friends & Family (150: the original 100 with their ids untouched, plus 50), Couples (75) and Spicy (50, 18+, behind the 0438 gate). 255 cards. The picker screen came back on its own, because decks are data. Deck icons are named in the JSON and resolved through a static map. The founder chose the split by *who is in the room* rather than by topic, and kept it to three on purpose so no deck runs thin — don't add a fourth without the content to fill it. `tests/wouldYouRather.test.ts` now pins the three decks, the gate on Spicy only, a 40-card floor per deck, and that the first 100 ids never move; `scripts/drive-would-you-rather.mjs` plays every deck, including the PIN.)
>
> **2026-09-23** (**Would You Rather goes live, on a rewritten deck.** It had sat in the hidden Coming Soon tab because the founder wasn't happy with its cards. The five old decks (160 cards, mostly well-worn internet staples, each with a "psychoanalysis" roast line) are replaced by ONE general deck of 100 new cards in `src/data/would_you_rather.json`; the psychoanalysis panel is gone. The split percentages stay, but they are **authored estimates, not votes** — the old footnote claimed "global player votes", which was never true, and the card now says so. Decks are data (name/tagline/colour in the JSON); with one deck the picker is skipped. It now sits first in the NEW tab. Pinned by `tests/wouldYouRather.test.ts` (THE DILEMMA BAR) and `scripts/drive-would-you-rather.mjs`.)
>
...
- **Games / NEW tabs:** the newest games (`NEW_GAME_IDS` in `App.tsx` — Would You Rather, The Line, Target, Shortlist, Echo, Ballpark, House Rules) are split out of the main list into their own **NEW** tab above the filter pills, so the front page stays a short list. The tab strip hides itself while a search is running, because **search deliberately spans both tabs** — a game the user typed the name of must never come back "no games match" because it sits on the other tab. Moving an id out of `NEW_GAME_IDS` returns that game to the main list, nothing else to change. This is separate from the `SHOW_TABS` Play Now / Coming Soon bar above. **Dev drives must click the NEW tab** (`button[aria-label="New games"]`) before those seven game cards exist in the DOM — `drive-games.mjs` and the per-game drives already do.
...
| Roast Me | `ROAST` | AI roast from uploaded image | Gemini (image + text) | **Adult-gated** (in `ADULT_GAME_IDS`, PIN 0438). 12 seasonal themes, two modes — SINGLE (one theme) and **COLLAGE** (four random themes as one composite image). See the Roast Me theme system below. Uses image gen, can't swap to Claude |
| Imposter | `IMPOSTER` | Find the fake among friends | Gemini | |
| Would You Rather | `WOULD_YOU_RATHER` | Paired dilemmas | Local static data | **Live since 2026-09-23.** 275 hand-written cards in `src/data/would_you_rather.json`, in **three decks split by who is in the room**: **Friends & Family** (`general`, 150, must stay safe with parents and kids at the table), **Couples** (75, relationships and date-night adventures, not explicit) and **Spicy** (50, sex/dating, `adult: true` → the 0438 PIN). Dealt 10 a round with per-deck session dedupe, then a round-end screen ("went with the crowd on N of 10", Ten more / Change deck). **Two modes** on the picker: **Classic** (one tap, reveal) and **🔥 Hot Seat** (2+ names via `TeamRosterRow`; the seated player picks in secret → the room taps its call → reveal; seat rotates; round end ranks who fooled the room most). The secret pick leaves no visible trace until the room has called it. Tap a side → both splits show and one line says whether you're with the majority. **The splits are authored estimates, not recorded votes** — the card's footnote says exactly that; never relabel them "player votes". The old per-card "psychoanalysis" roast was removed at the founder's request. Decks are data (`name`/`tagline`/`icon`/`color`/`adult` per category; `icon` resolves through the static `DECK_ICONS` map) — with a single deck the picker is skipped. **Card ids are session-dedupe keys, prefixed per deck (`g_`/`c_`/`s_`); append new cards, never renumber.** Every card carries a `tag` (topic) and `hooks` (which of edgy / funny / exciting / revealing it clears — THE FUN BAR, at least one required; a card that is merely a real choice doesn't ship). THE DILEMMA BAR (`tests/wouldYouRather.test.ts`): exactly these three decks with only Spicy gated, ≥40 cards per deck, the first 100 general ids unchanged, splits sum to 100 and sit within 20–80 (a 90/10 card is not a dilemma), no option text repeats across ALL decks, options ≤80 chars, and every card names ≥1 hook. The `wyr_batch` AI handler still exists but nothing in the game calls it. |
...
**Picker layout.** Three tiles across, four rows at twelve themes. The catalog's 8-character label cap is sized for THIS column width — narrowing the grid again means lowering it. At four rows the upload card falls below the fold on a 667px-tall phone; that is survivable only because the page scrolls, so don't add `overflow-hidden` to the containers around this screen (`scripts/drive-roast-lab.mjs` checks the upload button stays reachable at 375x667).
...
**COLLAGE mode.** Roast Me's picker has two tabs. SINGLE is the original flow — one theme, one image. **COLLAGE** renders FOUR themes as ONE composite image: output images bill per image, not per pane, so a four-look sheet costs one generation (~$0.134 at 2K) rather than four (~$0.54). Panes land at ~1024px, which is what a single roast shipped at before this work. The four are a random draw from whatever is in season (`pickCollageThemes`, Fisher-Yates over `availableThemes()`), shown as chips with their quadrant positions before you spend anything, and re-drawn by Reshuffle and on every Redo — with twelve themes that is 495 possible sets, and the variety is the point. Captions are four SEPARATE `generate_roast` calls, one per theme, because merging four comic voices into one request dilutes all four; they are cheap text calls against an expensive image one. Rock's caption is pinned to `punk` in collage because its COMPOSITE directive is the punk scene specifically. `CollageResult` slices the sheet with `cropQuadrants` and a tap opens one panel full-bleed with its own caption — pure client-side cropping, no second generation. If slicing fails the whole sheet is shown undivided rather than losing an image already paid for.
...
src/components/games/roast/CollageResult.tsx # COLLAGE result: 2x2 sheet, tap a panel to open it full-bleed
...
tests/wouldYouRather.test.ts         # vitest: THE DILEMMA BAR — WYR's three decks, gate on Spicy only, stable ids, 20–80 splits, no repeated options (run by CI)
...
scripts/drive-games.mjs              # DEV-only headless-browser drive: opens the 19 Play Now games (7 via the NEW tab; 22 with --tabs), fails on console errors
...
scripts/drive-would-you-rather.mjs   # DEV-only: a round in each Would You Rather deck (Spicy via the PIN), card-by-card against the JSON
