...
> **Last reconciled with code:** 2026-07-07 (shipped **Roast Central** — Roast Me v2 Phases 1 AND 2 built as a separate game per founder direction, existing Roast Me untouched; deleted the dead `roast_or_toast` API path. Phase 2 added the $0 canvas poster gallery (`src/services/roastCards.ts`) + session recap card. **Phase 3 (Roast Battle)** shipped: a Solo/Battle toggle on Roast Central's SETUP opens `RoastBattleGame.tsx` (pass-and-play roster → capture → per-player roast+poster → reveal → vote-not-self → `EndScreen` + Game Night + recap); the persona/format/spice library + photo/fallback/frame helpers were extracted into `src/components/games/roastShared.ts`, shared by both roast games. No new AI plumbing — Battle rides `roast_observe` + `roast_text_batch`. **Phase 4 (Toon Studio)** shipped same-day: `edit_image` gained a `style` key + per-style model tiers (6 flash on `gemini-2.5-flash-image`, 2 ✨PRO on `gemini-3-pro-image`; legacy Roast Me calls untouched), client gallery in `roast/ToonStudio.tsx` with a 3/day localStorage ration (failures refunded). Prior reconciles: 2026-07-06 added `docs/ROAST_ME_V2_PLAN.md`; 2026-07-04 Phase 1 hardening + Phase 2 engagement layer). If you're reading this and something in the codebase doesn't match what's described here, **the code is the source of truth** — please update this file in the same PR that makes the change.
>
...
| **Roast Central** | `ROAST_CENTRAL` | Persona roast deck from one photo | **Claude → Gemini fallback** (text), Gemini (vision) | Roast Me v2 **Phase 1** of `docs/ROAST_ME_V2_PLAN.md`. Observe-once engine: photo downscaled ≤1024px client-side → one `roast_observe` vision pass (cached in sessionStorage by photo hash) → text-only `roast_text_batch` calls (5 roasts each, ~$0.003). 6 personas × 5 formats × 3 spice (Extra behind the 0438 gate; child detected → forced Hype Man + mild, client AND server). Burn Book (localStorage `roast_central_burnbook`), offline fallback deck (`roast_central_fallback.json`, dynamic-imported), 60-batch/2h session cap. **Phase 2:** every roast **auto-renders into a designed poster** (the player's photo + the burn, composited on canvas — $0, offline) as the DEFAULT deck visual — no opt-in click. Random frame per card; a dice + frame strip re-roll it; tap to enlarge (lightbox); Share/Save the poster. 5 templates (WANTED/tabloid/yearbook/trading-card-with-seeded-stats/certificate) in `src/services/roastCards.ts` (composes `shareCard.ts` primitives; `renderRoastCardWithImage` is the sync per-swipe draw) + a session recap card via `shareResultCard`. **Phase 3 — Roast Battle:** a **Solo / Battle** toggle on SETUP switches into `RoastBattleGame.tsx` (2–8 players via the shared `TeamRosterRow`). Pass-and-play: one shared persona/format/spice (host picks or "Surprise me"; Extra behind 0438) → each player snaps a face → one roast + poster per player (offline deck if the API's down) → pass-and-reveal → **vote the hardest burn, can't vote self** → `EndScreen` (accent `indigo`) → `gameNightService.reportResult('ROAST_CENTRAL', …)` + `statsStore` + `shareResultCard` recap. Shared persona/format/spice library + photo/fallback/frame helpers live in `src/components/games/roastShared.ts` (imported by both games); reveal posters use the async `renderRoastCard` (decodes each player's photo internally). **Phase 4 — Toon Studio** (`roast/ToonStudio.tsx`): the rationed caricature tier — gold wand on the deck (+ "Trophy toon" for the Battle winner) opens a style sheet; user picks 1 of 8 styles BEFORE generating (`generateToonImage` → `edit_image` with a `style` key; server library in `handlers-image.ts` owns prompt + tier — 6 flash styles on `gemini-2.5-flash-image`, 2 ✨PRO on `gemini-3-pro-image`). **3 caricatures/day** (localStorage `roast_central_toons`; failures refunded), kid-detected photos see only kid-safe styles. Legacy Roast Me `theme` calls unchanged (still Pro). |
...
| `api/_lib/roast-prompts.ts` | Roast Central prompt library: persona/format/spice/context blocks + `RoastObservations` type. Ids must stay in sync with the client's `PERSONAS`/`FORMATS`/`SPICES` in `src/components/games/roastShared.ts` (shared by Roast Central + Roast Battle). |
...
- **Gemini 3 Pro Image** (`gemini-3-pro-image`) — image editing for Roast Me caricatures + the 2 ✨premium Toon Studio styles (in-image text rendering). (Bumped 2026-07-02 from the preview model, which was slated to retire ~2026-07-17 — same failure class as the 2.0-flash text shutdown that broke prod on 2026-06-01. Lesson: never leave a `-preview` model id in production; check model retirement dates monthly.) Image gen also depends on the Gemini billing account being funded — an empty balance 404s/errors it.
- **Gemini 2.5 Flash Image** (`gemini-2.5-flash-image`) — the DEFAULT Toon Studio tier (Roast Central Phase 4): ~$0.039/image, a few seconds per generation (safe on Vercel Hobby's 10s limit). Stable id per the retirement lesson above; **verify it generates in prod after first deploy** (added 2026-07-07 from this offline dev env — id could not be exercised against the live API here).
...
5. **Image-gen rationing is client-side only** (Toon Studio's 3/day lives in localStorage — trivially reset by clearing storage). The plan's server-side per-IP daily limit (`docs/ROAST_ME_V2_PLAN.md` §6) needs a persistent store (e.g. Upstash Redis free tier) the founder must provision; until then a determined user can burn image quota. Fine for a party app, fix before any paid/marketing push. Also: Toon Studio's 2 ✨PRO styles run 15-30s on `gemini-3-pro-image` and can hit Vercel Hobby's 10s function limit — the client treats that as a normal failure (allowance refunded), but if PRO styles never succeed in prod, that's why (upgrade to Pro tier or convert the route to an Edge Function).
...
│       ├── RoastCentralGame.tsx     # Roast Me v2 solo deck (Phases 1+2); hosts the Solo/Battle toggle
│       ├── RoastBattleGame.tsx      # Roast Me v2 Phase 3 — pass-and-play party battle
│       └── roastShared.ts           # Shared roast library + photo/fallback/frame helpers (both roast games)
├── contexts/
...
scripts/drive-roast-battle.mjs       # DEV-only Roast Battle flow drive (offline fallback path, dumps poster PNGs)
scripts/drive-roast-toon.mjs         # DEV-only Toon Studio drive (stubbed API: success/reveal + failure/refund paths)
```
