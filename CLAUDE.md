...
> **Last reconciled with code:** 2026-09-05 (four new brain games shipped as **top-level Home games**. `BallparkGame.tsx` + `ballpark.json` — "Ballpark", estimation/calibration. `EchoGame.tsx` + `echo.json` — "Echo", a memory chain with a sabotage layer. `ShortlistGame.tsx` + `shortlistEngine.ts` + `shortlist.json` — "Shortlist", **the app's first cooperative game** and its first generated content. `TargetGame.tsx` + `targetEngine.ts` — "Target", six numbers and a three-digit target, and **the first game with no content file at all**: every puzzle is generated AND solved at deal time. Both engines are guarded in CI by `tests/shortlistEngine.test.ts` and `tests/targetEngine.test.ts`. Also added `scripts/drive-ballpark.mjs`, `drive-echo.mjs`, `drive-shortlist.mjs` and `drive-target.mjs`, the `target` / `layers` / `fingerprint` / `calculator` keys to `getIcon`, all four games to `scripts/drive-games.mjs` and to Game Night's eligible list, and — fixing two documented-intent drifts found on the way — House Rules' stats id (`house_rules` → `HOUSE_RULES`) and its own absence from that eligible list.) If you're reading this and something in the codebase doesn't match what's described here, **the code is the source of truth** — please update this file in the same PR that makes the change.
>
...
- **Tests:** `npm test` → vitest render smoke test + the Shortlist and Target engine invariants (`tests/App.smoke.test.tsx`: splash → home menu through the real module graph; jsdom, fetch/matchMedia stubbed in `tests/setup.ts`). Config in `vitest.config.ts` (deliberately separate from `vite.config.ts`).
...
│   └── games/                       # One file per game (incl. JumbleGame = "Scramble", IntimateDiceGame, TheTellGame, NerveGame, HouseRulesGame, BallparkGame, EchoGame, ShortlistGame, TargetGame)
├── contexts/
...
scripts/drive-games.mjs              # DEV-only headless-browser drive: opens the 17 Play Now games (21 with --tabs), fails on console errors
