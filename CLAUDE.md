...
> **Last reconciled with code:** 2026-08-23 (added `src/components/games/TheTellGame.tsx` + `src/data/the_tell.json` — "The Tell", a two-player secret-agenda game reached from the Truth or Drink category screen. Also added `scripts/drive-the-tell.mjs`, and replaced the hard-edged decorative "blob" pattern with a radial wash in the files it touched — see `notes/04-decorative-blob-hard-edge.md`.) If you're reading this and something in the codebase doesn't match what's described here, **the code is the source of truth** — please update this file in the same PR that makes the change.
>
...
│   └── games/                       # One file per game (incl. JumbleGame = "Scramble", IntimateDiceGame, TheTellGame)
├── contexts/
