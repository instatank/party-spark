...
> **Last reconciled with code:** 2026-08-24 (added `src/components/games/NerveGame.tsx` + `src/data/nerve.json` — "Nerve", two-player chicken, the second game reached from the Truth or Drink category screen after "The Tell". Also added `scripts/drive-nerve.mjs` and `notes/05-invariant-held-by-constructor-only.md`.) If you're reading this and something in the codebase doesn't match what's described here, **the code is the source of truth** — please update this file in the same PR that makes the change.
>
...
- **Browser regression drives (dev-only, not in CI):** `scripts/drive-games.mjs` (opens the 12 Play Now games headless — all 16 with `--tabs` — and fails on console errors) and `scripts/deep-drive.mjs` (countdown/expiry/score flows in the 6 timer games) and `scripts/drive-the-tell.mjs` (plays a full 12-round game of The Tell and asserts every outcome branch) and `scripts/drive-nerve.mjs` (plays a best-of-3 of Nerve and asserts the ladder-escalation invariant — run it a few times, the ladders are randomised) against `npm run build && npx vite preview --port 4173`. See `notes/02-browser-regression-drive.md` for the gotchas. Run these after touching shared game code.
...
│   └── games/                       # One file per game (incl. JumbleGame = "Scramble", IntimateDiceGame, TheTellGame, NerveGame)
├── contexts/
