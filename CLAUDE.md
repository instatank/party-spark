...
> **Last reconciled with code:** 2026-09-07 (**Charades got a second round format and a second deck.** The shipped dataset was built for a rapid-fire loop — a third of it single words — but the way the game actually gets played at this table is ONE clue with the whole 30/60s to land it. So `src/data/charades_clues.json` (448 clues, 6 packs, each tagged with a KIND announced to the room and a 1-3 difficulty) now sits beside `games_data.json`, reached through `src/services/charadesClues.ts`, and `CharadesGame` has a **Rapid Fire / One Clue** switch on SETUP with its own brief → act → verdict loop. Pinned in CI by `tests/charadesClues.test.ts` (THE ONE-CLUE BAR) and out of CI by `scripts/drive-charades-one-clue.mjs`. The old deck was also de-duplicated: `mix_movies` used to store all of Hollywood + Bollywood again and `family_mix` all three of its sub-pools, so 963 stored strings became 628 while Family Mix's dealt pool went 61 → 191 — which also closed a quiet bug where a starved Family Mix fell through to an AI prompt that only ever asked for movie titles. Also 2026-09-07: **optional round clocks** — Ballpark ships with a 45s bracket clock ON, The Line and Shortlist have one OFF by default, and every one of them is editable or fully off from the shared `TimerSetting` chip; see the timer table under Design System for which games get a clock and what each does at zero.)
>
...
|---|---|---|---|---|
| Charades | `CHARADES` | Act it out silently | Gemini (Rapid Fire refills only) | **Two round formats, two decks** — the switch is on SETUP and persists (`charades_format`). **Rapid Fire** is the original loop: one clock, get through as many cards as you can, drawn from `games_data.json`'s short word list (Movie Mix / Family Mix / Bollywood / Hollywood). **One Clue** is the way it gets played at a table: one card per turn with the whole 30/60s to land it, drawn from a *separate* deck (`src/data/charades_clues.json` via `src/services/charadesClues.ts`) whose clues are built to hold a person's minute — scenes with a turn in them, sayings, films with something to do. Its loop is BRIEF (actor-only reveal, card blurred until they tap — they need a beat to plan) → PLAYING → REVEAL (always shows the clue, hit or miss) → next actor, with 3/5/7 clues each and turns round-robin off a schedule dealt up front, so the difficulty curve is a property of the whole match. **Do not merge the two decks**: a card that reads "Don" is three seconds of charades however long the clock is, which is the whole reason the second deck exists. Round timer via the shared `TimerSetting` chip (default 60s, persisted, shared by both formats). |
...
│   ├── charadesClues.ts             # Charades One Clue deck loader + dealClues (opens easy, ends hard) — separate from games_data.json
...
tests/charadesClues.test.ts          # vitest: THE ONE-CLUE BAR — the Charades one-clue deck can't drift back to a list of one-word cards (run by CI)
...
scripts/drive-charades-one-clue.mjs  # DEV-only full One Clue match (deck provenance, blur-before-reveal, timer expiry, independent tally)
