...
> **Last reconciled with code:** 2026-09-20 (**Roast Me: LINKEDIN retired, PASSPORT added.** Founder call — the corporate-headshot joke never landed. Its replacement keeps the "official document" instinct and swaps the source material: an extreme close-up of a passport data page, the flat unsmiling photograph filling the frame with the printed data column bleeding off one edge. Still **12 themes** (one out, one in), so the picker layout and the season tests are unchanged. Two things in that prompt are load-bearing and pinned by `tests/roastThemes.test.ts`: the **zoomed framing** (ask a model for "a passport" and it draws the whole booklet on a desk, where the face is a postage stamp and the roast has nothing to land on) and the **fictional-issuer clause** (we are putting a real face on an identity document; a convincing replica of a real country's data page is a forgery template whatever the caption says — an invented country is also funnier, so nothing is traded away). LINKEDIN follows the file's existing retirement contract: gone from the picker, prompts kept server-side so PWA-cached phones that still POST `linkedin` get the roast they asked for rather than a 500. **The prompt itself is unverified against the image model** — that needs a real key and a real photo, so it can only be judged on a deployment; `#roast-lab` is the instrument.)
>
