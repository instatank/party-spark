...
| Game | GameType | Mechanic | AI | Notes |
|---|---|---|---|---|
| 5 Alive | `FIVE_ALIVE` | Name N in N seconds vs a buzzer | None (offline) | Currently listed under `comingSoonGameIds` in `App.tsx` while it's tested. 5 descending rounds — name 5/4/3/2/1, timed 6/5/4/3/2s (extra second to read the clue) — perfect-round bonus, judge tallies. Easy + Hard category pools in `src/data/five_alive.json`. End-of-round bell + tick are synthesized via Web Audio (no bundled assets); the players screen is the landing screen, difficulty is picked after. Also has a "Just Play" no-scoring mode like TOD. **Single game (5 rounds) per player** — the "rounds-per-player ×2/×3" option from the handoff spec was deliberately left out of the minimal PR. |
| Linked | `LINKED` | One connector word pairs with all 3 clues (e.g. water/down/rain → FALL) | None (offline) | Listed under `comingSoonGameIds` in `App.tsx` while it's tested. Two modes: **Pass and Play** (60s per player, self-reported "Got it!"/"Skip", leaderboard) and **Just Play** (no timer, group shout, Reveal → Next loop). Easy (66) + Hard (16) puzzle pools in `src/data/linked.json` — shape `{ clues: [3], answer, position? }` (`position` optional, defaults `'suffix'`; bundled data is all-suffix). Buzzer + tick + got-it ding synthesized via Web Audio. Per-puzzle session dedupe via `SessionManager`. Deliberately out of the minimal PR: AI custom puzzle decks, daily-puzzle/streak, hint system. |
