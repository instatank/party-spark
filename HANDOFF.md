...
| Working branch this session | `claude/partyspark-the-line-sr3hrb` (The Line) |
| Latest production commit | "The Line" (2026-09-06) — sequencing, the fifth new brain game. Always confirm with `git log origin/main`. |
...
6. **Charades** — **two formats, two decks** (2026-09-07). *Rapid Fire* deals from `games_data.json`: Hollywood (200) / Bollywood (200) / mix_movies extras (37) / Everyday Actions (60) / Around the House (45) / The Zoo (46) / family_mix extras (40). Those seven are **storage**, not the picker — the game unions them into four player-facing decks, so Movie Mix deals 437 and Family Mix 191. *One Clue* deals from a separate file, `src/data/charades_clues.json` — 448 clues over 6 packs (Situations / Screen / Desi / Sayings / People & Jobs / Family), each tagged with a kind and a 1–3 difficulty. The two decks are deliberately not interchangeable.
...
16. **The Line** (2026-09-06) — 1–8 players, solo-capable. Sequencing: every card hides a number and you never state it, you only tap the GAP in the shared line where it belongs. Right locks in and reveals; wrong is discarded, the truth shown, and you draw a replacement. 208 cards over 4 single-axis decks (How Tall / How Fast / How Long Ago / How Heavy). Engine + invariant in `src/services/lineEngine.ts`, guarded in CI by `tests/lineEngine.test.ts`.
17. **The Forecast** (Compatibility Test) — 3 modes (friends / couples / bunny), 30 Qs each. Adult-gated.
18. **Imposter** — pass-and-play deduction.
...
11. ~~**Screen mirroring / viewer mode**~~ — **revisited and resolved 2026-09-06.** The old estimate (~1–2 weeks, needs a realtime backend) priced *mirroring* — replicating one phone's screen onto another. That turned out to be both the expensive build and the worse product: every game holds 13–24 scattered `useState` hooks with no reducer, and half the roster (The Tell, Imposter, Charades, Ballpark's own HANDOFF) is built on the phone being **private**. What shipped instead is a **shared seed + per-player results inbox** — see the Multiplayer section of `CLAUDE.md`. Scramble head-to-head and Ballpark live are wired; Target and The Line are the obvious next two. A read-only second screen (a TV showing the current card) is still unbuilt and would be cheap on the same room primitive.
