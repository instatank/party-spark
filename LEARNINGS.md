...
### 2026-09-07 — Third time on the same trap, because the fix I wrote down was a habit and not a tool
- What happened: the new Charades drive checked that each clue's little category label matched the deck file — "Situation", "Movie", "Saying". Six failures out of six. The app was right every time: that label is styled to display in capitals, and the browser hands text back *after* styling, so the script compared "Situation" against "SITUATION". This exact trap is already written down twice in this file (23 Aug and 5 Sep) and once in `notes/04`, and the recorded fix was "build the guard into the tool rather than remember it at each use — one shared helper that compares text case-insensitively, used everywhere from the start". I then wrote a brand-new drive script from scratch with its own private text-matching helper, because there is no shared one in `scripts/` — twelve drive scripts, twelve separate copies. The lesson had been recorded three times and implemented zero times.
- Concept: writing a lesson down is not the same as removing the trap, and the gap between them is invisible because the note *reads* like the problem is handled. A lesson whose fix is "remember to do X" will be re-learned on every new file, because a new file starts with nobody remembering anything. The only fixes that hold are the ones that change what the next person picks up by default — a shared helper they import instead of retyping, a template the new script is copied from, a stable hook in the app itself. The tell is in the wording: if a note's fix is a verb aimed at a human ("remember", "check", "be careful"), it hasn't been fixed yet. This session also proved the *other* half works — the drive scripts all now click by `aria-label` rather than visible text, which was the 6 Sep lesson, and it did hold, because that fix put something in the app instead of in someone's head.
- In my words: (pending — answer at next wrap)
- Where else: (pending — answer at next wrap)
- Quiz question: You've written the same lesson down three times and hit it a fourth. What does that tell you about the *shape* of the fix you wrote, and what kind of fix would have actually stopped it?
- Internalized: no

### 2026-09-07 — The Family deck ran out and quietly started dealing movies
- What happened: Charades has a Family Mix deck of everyday actions, household objects and animals. It held 61 items and the game deals a batch of 30, so by the second or third round the local pool was empty — at which point the code falls through to asking the AI for more. That request's prompt read "Generate a list of N popular and recognizable Movie Titles for a game of Charades. The category is: family_mix." The category name was pasted into a sentence that had already decided the answer was film titles. So the family deck, once exhausted, started serving Hollywood films to a table playing with children. No error, no warning, no failing test — the fallback did precisely what it was written to do. I only found it because I was counting pool sizes for an unrelated reason and happened to read the prompt next to them.
- Concept: a fallback is a *second implementation of the same requirement*, and it is the one nobody looks at, because it only runs in the state nobody tests — the state where the main path has run out. Two habits come out of this. First, size every content pool against how fast the game eats it: "61 items" means nothing, "61 items against a batch of 30" means the fallback is doing the real work by round three. Second, when you audit content, read the code that generates more of it — the generator is part of the dataset, and it can disagree with the file it's backing up without anything ever going red.
- In my words: (pending — answer at next wrap)
- Where else: (pending — answer at next wrap)
- Quiz question: A backup path in your app runs only when the main one is exhausted, and it serves the wrong kind of thing entirely. Why would no test catch that, and what's the cheap way to spot the risk before it bites?
- Internalized: no

### 2026-09-10 — Built four screens the founder then asked me to delete
- What happened: the brief was "the way we prefer to play is one clue and a 30s/60s timer to get that one clue, so the clues can be longer, harder — please review and revise the dataset." I heard the *play pattern* and built machinery for it: a brief screen where the card sits blurred until the actor taps Reveal, a per-turn verdict screen, a schedule of turns dealt up front, a 3/5/7 clues-each setting, and a difficulty curve so a match opened easy and ended hard. All of it worked, all of it was tested, and the very next instruction was "you've added more mechanics to the gameflow — keep it simple, keep the gameflow as it was. Card reveals clue, correct/incorrect leads to next card. It's efficient that way, with fewer steps." Removing it took 260 lines out of the file and left the feature better.
- Concept: the sentence described what the founder *wanted from the cards*, not a request for new screens — but "we play it this way" reads like a mechanic, so I built the mechanic. The tell I missed was that everything I added sat **between the card and the next card**, and the existing loop had nothing there on purpose. A game loop that has survived a dozen games is a decision someone already made; adding a step to it needs its own reason, not a side effect of a content brief. The cheap check before building: separate what was asked for (better clues) from what I inferred (a different way of playing them), and only the first one is the deliverable unless the second is asked for out loud. Everything I inferred was also the part that had to come back out.
- In my words: (pending — answer at next wrap)
- Where else: (pending — answer at next wrap)
- Quiz question: Someone describes how they like to use your product and asks you to improve the content. What are the two different things you could build from that sentence, and which one is the deliverable?
- Internalized: no

### 2026-09-10 — The prediction from three days ago came true on the very next script
- What happened: the new Charades drive froze for thirty seconds and died on "Navigation timeout". Cause: the play screen deliberately arms a browser "are you sure you want to leave?" guard so a real player can't lose a round by hitting refresh, and in a headless browser that hangs any page reload until something answers the prompt. The fix is one line — accept the dialog — and it is already sitting in five other drive scripts in the same folder, and the trap is already written up in this file from 23 August. I wrote a new script from scratch and re-acquired it, which is *exactly* what the card I wrote three days ago predicted would keep happening.
- Concept: last session's lesson said a fix shaped as "remember to do X" gets re-learned on every new file, because a new file starts with nobody remembering anything. This is that claim being confirmed inside one week, on a different trap, in the same folder — which makes it evidence rather than a theory. Twelve drive scripts each re-implement the same six helpers (launch, console filtering, click-by-text, dialog handling); every new one starts from a blank page and pays for the same lessons again. The concrete debt is now nameable: there is no `scripts/_drive-kit.mjs`. Until there is, the honest expectation is that drive number thirteen hits a trap that drives one through twelve already solved.
- In my words: (pending — answer at next wrap)
- Where else: (pending — answer at next wrap)
- Quiz question: You wrote down "this will keep happening until the fix lives in a shared tool" and then it happened again three days later. What is the actual outstanding task, and why does writing the lesson a fourth time not count as doing it?
- Internalized: no

### 2026-09-14 — Loosening a type to be honest about the data quietly switched off the check that was guarding three copies
- What happened: Roast Me's themes moved from a hardcoded `switch` to a data catalog, and the theme key's type went from a fixed list of six names to plain `string`. That was the right call — themes now rotate with the season, so a fixed list would be wrong the moment one retired. But somewhere else in the app a map of theme labels was declared as "a record keyed by a theme", and the moment "a theme" stopped meaning six specific names and started meaning any word, that map stopped being checked for completeness. It had six entries. There were now twelve themes. The compiler was satisfied, every test passed, the browser drive passed, and seven of the twelve themes shipped to production with a blank badge. I only found it by fetching the JavaScript the live site was actually serving and reading it.
- Concept: a type that lists its members is doing two jobs — describing the data AND forcing every place that enumerates it to stay complete. When you widen that type because the data really is open-ended, you keep the first job and silently lose the second, everywhere at once, with no error to tell you which places just became unguarded. The fix is not a better type; it's having fewer places. I had already caught this shape of problem once this session and written a test for it — but the test guarded the two copies I knew about and knew nothing about the third one living inside a component. Deleting the third copy is what actually fixed it: a list that exists once cannot disagree with itself.
- In my words: (pending — answer at next wrap)
- Where else: (pending — answer at next wrap)
- Quiz question: You change a setting's type from "one of these five names" to "any text", because the five names are about to become data. Nothing breaks and nothing turns red. What did you just switch off, and where do you go looking for the damage?
- Internalized: no

### 2026-09-14 — Drive script thirteen, and it paid for two lessons the first twelve had already bought
- What happened: I wrote a new browser-drive script for the Roast screens. It failed twice on things already solved elsewhere in the same folder. First, it walked into the adult PIN gate and reported "theme grid not found", because Roast Me is one of three games behind that gate and the script didn't know to type 0438. Second, it flagged blocked Google-Font requests as app errors, because it filtered console errors by reading the message text — and "Failed to load resource" messages carry their address somewhere else entirely, so the filter never matched. Both fixes already exist in sibling scripts. The card written on 10 September predicted this in as many words: "the honest expectation is that drive number thirteen hits a trap that drives one through twelve already solved." This is drive number thirteen. I hit two.
- Concept: this is now the third session confirming the same claim, which makes it a measurement rather than an opinion — a lesson recorded as a habit gets re-learned by every new file, because a new file begins with nobody remembering anything. The outstanding task has had a name since 10 September: there is no `scripts/_drive-kit.mjs` holding the shared launch, console-filtering, click-by-label, dialog-handling and PIN-clearing helpers that all thirteen scripts re-implement. I did not build it this session either — I fixed my own two traps locally and moved on, which is precisely the behaviour the card describes. Writing this card a fourth time still does not count as doing it, and the next drive will pay again.
- In my words: (pending — answer at next wrap)
- Where else: (pending — answer at next wrap)
